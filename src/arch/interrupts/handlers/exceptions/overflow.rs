//! A file that handles the Overflow exception. The handle function is called automatically 
...
/// A function which is registered to handle the Overflow exception. Since it is an
...
pub fn handle(_info: *const context::Context) {
    oxid_err!("Overflow exception recieved. Halting the system.");
    unsafe { proc::halt(); }
}
