//! A file that handles the Math fault exception. The handle function is called automatically 
...
/// A function which is registered to handle the Math fault exception. Since it is an
...
pub fn handle(_info: *const context::Context) {
    oxid_err!("Math fault (x87 floating-point) exception recieved. Halting the system.");
    unsafe { proc::halt(); }
}
