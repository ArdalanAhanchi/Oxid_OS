//! A file that handles the Bound range exception. The handle function is called automatically 
...
/// A function which is registered to handle the Bound range exception. Since it is an
...
pub fn handle(_info: *const context::Context) {
    oxid_err!("Bound range exception recieved. Halting the system.");
    unsafe { proc::halt(); }
}
