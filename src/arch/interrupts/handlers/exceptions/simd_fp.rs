//! A file that handles the SIMD Floating point exception. The handle function is called 
...
/// A function which is registered to handle the SIMD Floating point exception. Since it is an
...
pub fn handle(_info: *const context::Context) {
    oxid_err!("SIMD Floating point exception recieved. Halting the system.");
    unsafe { proc::halt(); }
}
