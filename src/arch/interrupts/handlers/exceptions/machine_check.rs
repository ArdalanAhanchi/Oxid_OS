//! A file that handles the Machine check exception. The handle function is called automatically 
...
/// A function which is registered to handle the Machine check exception. Since it is an
...
pub fn handle(_info: *const context::Context) {
    oxid_err!("Machine check exception recieved. Halting the system.");
    unsafe { proc::halt(); }
}
