//! A file that represents the x86_64 page-map level 4 structure. It is the top-most structure in
//! the 4-level paging schema. All the other tables will be accessed through here.
//!
...
/// Define an entry for PML4 and implement all the general functions for it using the defined macro
/// in general_entry. In this case, all the required entry functions are implemented. Here is the 
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct PML4Entry(usize);    // PML4 Entry that contains 64-bits of data.
impl_general_entry!(PML4Entry); // Implemented functions with general entry macro.
impl_make_table_if_not_present!(PML4Entry, PDP);

/// Define the PML4 table with an address and number of entries.
pub struct PML4 {
    virt_addr: usize,
    num_entries: usize,
}

impl_general_table!(PML4, PML4Entry);

impl PML4 {
    /// A method which accepts a virtual address, and returns the index within PML4 for that addr.
    /// this is based on the architecture definitions which is defined in the AMD64 manual volume 2
    /// 136. This specifically uses bits 39 to 47 (inclusive).
    ///
    /// # Parameters
    /// `virt_addr` : The virtual address which we're trying to map.
    ///
    /// # Returns
    /// The index within the PML4 for that specific address.
    pub fn get_idx(virt_addr: usize) -> usize {   
        // Shift right to start at bit 39, then bitmask it to only get the first 9 bits.
        (virt_addr >> 39) & 0x1FF
    }
}
